...
def main(model="fpca", force=False):
    ...
    for folder in folders:
        ...
        for subf in [subf for subf in os.listdir(path) if os.path.isdir(os.path.join(path, subf))]:
            ...
            data_path = os.path.join(path, subf, f"{subf}_merged_new.csv") if folder != START[typ] else os.path.join(
                path, subf, "merged_X.csv")
            if model.lower() == 'fpca':
...
def train_OCSVM_model(data_path):
    ...
    train_df, test_df = train_test_split(
        train_df, test_size=0.25, random_state=123)
    ...
    grid_search = GridSearchCV(
        OneClassSVM(), param_grid, scoring=custom_scorer(label), n_jobs=-1)
