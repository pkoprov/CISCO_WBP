...
name = input("Name of printer? ")
try:
    os.mkdir(f'./data/Kernels/{datetime.now().date().strftime("%Y_%m_%d")}')
except FileExistsError:
    pass
try:
    os.mkdir(f'./data/Kernels/{datetime.now().date().strftime("%Y_%m_%d")}/{name}')
except FileExistsError:
    pass
folder = f'./data/Kernels/{datetime.now().date().strftime("%Y_%m_%d")}/{name}'
...
while i < n+1:
    ...
    df.to_csv(f"{folder}/{name}_{time.strftime('%Y_%m_%d %H:%M:%S', time.localtime(time.time()))}.csv")
    plt.plot(data)
    plt.savefig(f"{folder}/{name}_{time.strftime('%Y_%m_%d %H:%M:%S', time.localtime(time.time()))}.svg")
    plt.close()
    print(f"Created {name}_{i}")
    i += 1
