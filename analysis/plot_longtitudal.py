    ...
    for folder in dir_list[1:]:
        ...
        for key in prediction.keys():
            ...
            results[key]['errors'].append(
                np.mean(prediction[key]['errors'], axis=0))
...
def plot_results(asset, n, version):
    ...
    if version == "old":
        threshold = np.mean([load_model(os.path.join(r".\data\Kernels", dir_list[0],
                            asset, f"{asset}_{lim}_fpca.pkl"))["threshold"] for lim in ["top", "bottom"]])
