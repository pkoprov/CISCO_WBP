...
plt.title(f"Random test {key} curve ({labels[test_ind[n]]}) projected on {label} {key} curve FPC")
plt.legend(loc = 'best')
plt.subplot(313)
n = np.random.choice(test_ind[y_test ==label])
test[n].plot(plt.gca(), label=f"original curve", color = "blue")
...
for asset in ["VF-2_1", "UR10e_A", "UR5e_N", "Bambu_M"]:
    ...
    for key in ["top", "bottom"]:
        model_threshold.append(load_model(fr"{model_dir}\{asset}_{key}_fpca.pkl")["threshold"])
    ...
    errors_wo_load_o = {asset: np.mean(result_wo_load[asset]["scores"], axis=0) for asset in [key for key in result_wo_load.keys()]}


    plt.figure(figsize=(8, 3))
    plt.plot(errors_w_load, 'v',label=f"{asset} with payload", color = "blue")
    plt.plot(errors_wo_load_a, 'o', fillstyle = "none", label=f" {asset} without payload", color = "blue")
    [plt.plot(errors_wo_load_o[asset], 'o', label=f"{asset} without payload", color = "red") for asset in errors_wo_load_o]
    plt.axhline(model_threshold, color="black", label="threshold")
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.title(f"Errors for {asset} with and without payload using updated model")
    plt.tight_layout()
